(function () {
  ...
  // makes the reported height monotonically non-decreasing, so the frame grows for a
  // result but never shrinks back — leaving a tall blank gap on phones.
  var elRoot = document.getElementById("wf");
  function postHeight(){
    try {
      var h = Math.ceil(elRoot.getBoundingClientRect().height);
      if (h > 0) parent.postMessage({ type: "carmallWiperHeight", height: h }, "*");
    } catch (e) {}
  }
